...
{
	// You have a certain amount of time before the lucky rng goes away
	// prevents trades from being always lucky
	if(level.round_number < 25)
	{
		...
		{
			// monkey bombs are given after the first box move on the 3rd hit
			if(!player has_weapon_or_upgrade("zombie_cymbal_monkey") && level.chest_moves != 0 && level.chest_accessed >= 3)
			{
				return "zombie_cymbal_monkey";
			}

			// bow is given after 5 box hits
			if(!player has_weapon_or_upgrade("crossbow_explosive_zm") 
			&& !any_player_has_weapon("crossbow_explosive_zm") && level.chest_accessed >= 4)
			{	
		...
		{
			if (!player has_weapon_or_upgrade("thundergun_zm") && !any_player_has_weapon("thundergun_zm") && level.chest_accessed >= 3)
			{
				return "thundergun_zm";
			}

			if(!player has_weapon_or_upgrade("zombie_black_hole_bomb") && level.chest_moves == 3 
			&& level.chest_accessed >= 2 && !player has_weapon_or_upgrade("thundergun_zm") && !any_player_has_weapon("thundergun_zm"))
			{
		...
		// Both maps
		if((level.script == "zombie_cosmodrome" || level.script == "zombie_pentagon") && !player has_weapon_or_upgrade("ray_gun_zm") && level.chest_moves != 0 && level.chest_accessed >= 3)
		{
			return "ray_gun_zm";
		}
	}
